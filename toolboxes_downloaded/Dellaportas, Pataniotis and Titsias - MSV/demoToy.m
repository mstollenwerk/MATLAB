...
mcmcoptions.adapt.minAdapIters = 300;
mcmcoptions.train.T = 10000;
...
for kk2=kk1+1:N    
   ...
   for j=1:2:size(Sigma,3)
      jj = jj + 1; 
      h = plot_gaussian_ellipsoid([2*jj cnt*3], (model.L(ind,:).*Weights(ind,:))*Sigma(:,:,j)*(model.L(ind,:).*Weights(ind,:))' + sigma2*eye(2));
