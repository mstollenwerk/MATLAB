function [model samples accRates] = mcmcTrain(model, PropDist, trainOps, Langevin)
% Inputs: 
...
%
   ...
   %     
      ...
      %    
        ...
        for t=1:model.T 
            ...
            if length(ind) > 0
               ...
               cholM = jitterChol(M);
               model.Ft(:,t) = sqrt(model.sigma2)*(cholM\randn(model.K,1)) + ((cholM\(cholM'\(Lobs'*model.Y(ind,t)))));
            else
      ...
      %    
          ...
          %ktemp = sum(Smatrix(:));
          %if isnan(ktemp) 
          %    save tempnan.mat model Smatrix PPhi;
          %    sdfdfssdf
          %end
      ...
      beta_post = model.priorSigma2.beta0 + 0.5*nansum(nansum( (model.Y - (model.L.*model.Weights)*model.Ft).^2 )); 
   ...
   %    
       for t=1:model.T
       %    
          indNans = find(isnan(model.Y(:,t)));
          indObs = find(~isnan(model.Y(:,t)));
          if length(indNans) > 0
          %    
    ...
    %
        ...
        if strcmp(model.useFactorModel, 'yes') 
           samples.LogL(cnt) = - (0.5*num_OfNonNans)*log(2*pi*model.sigma2) - nansum(nansum( (model.Y - (model.L.*model.Weights)*model.Ft).^2 ))/(2*model.sigma2);
        else
