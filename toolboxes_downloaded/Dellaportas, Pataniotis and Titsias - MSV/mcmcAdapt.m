...
if strcmp(model.useFactorModel, 'no') == 1
    ...
    for t=1:model.T
       ...
       if length(indNans) < model.N
           model.Ft(indNans,t) = mean(model.Y(indObs,t));
       else
...
while 1
    ...
    [model samples accRates] = mcmcTrain(model, PropDist, ops, Langevin);
    ...
    cnt = cnt + 1;
    % do not allow more than 300 iterations when you adapt the proposal distribution
    if cnt == 400
