function [mu,h] = narchcore(parameters, data, ar, ma, x, p, q, y, m, z, v, T)
%{
-----------------------------------------------------------------------
 PURPOSE:
 Estimation of conditional mean and variance of the 
 Nonlinear ARCH, Engle and Bollerslev (1986)
-----------------------------------------------------------------------
 USAGE:
 [mu,h] = narchcore(parameters, data, p, q, m, T)
...
for t = (m+1):T; 
   ...
   h(t,1) = parameters(2+z:2+z+p+q+v)'*[1; (abs((data(t-(1:p)) - mu(t-(1:p))))).^delta; h(t-(1:q));yy(t,:)'*ones((isscalar(y) < 1))];
end
