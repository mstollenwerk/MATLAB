function [mu,h] = nagarchcore(parameters, data, ar, ma, x, p, q, y, m, z, v, T)
%{
-----------------------------------------------------------------------
 PURPOSE:
...
-----------------------------------------------------------------------
 USAGE:
 [mu,h] = nagarchcore(parameters, data, p, q, m, T)
...
for t = (m+1):T; 
   ...
   h(t,1) = parameters(2+z:2+z+p+q+v)'*[1; ((data(t-(1:p)) - mu(t-(1:p)))./sqrt(h(t-(1:p))) + asym).^2; h(t-(1:q));yy(t,:)'*ones((isscalar(y) < 1))];
end
