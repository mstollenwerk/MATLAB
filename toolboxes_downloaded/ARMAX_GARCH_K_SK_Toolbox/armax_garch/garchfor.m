function [MF, VF, MC, VC] = garchfor(data, model, distr, ar, ma, p, q, max_forecast)
%{
-----------------------------------------------------------------------
 PURPOSE: 
...
elseif isequal(strcat(model), 'NGARCH')
    ...
    VF(1,1) = (parameters(2+z:2+z+p+q)'*[1; abs(resids(end-(0:(p-1)))).^delta; ht(end-(0:(p-1))).^delta]).^(1/delta); % 1-period ahead forecast
    for i = 2:max_forecast
