...
elseif ndims(data)==3
    ...
    data2d = zeros(T,k);
    for i=1:k
        data2d(:,i) = squeeze(sqrt(data(i,i,:)));
...
if stage == 1
    % Nothing to do, parameters are already constructed
elseif stage==2
...
isJoint = true;
isInference = true;
[ll,~,Rt,fcst_Rt] = dcc_likelihood(parameters,data,dataAsym,m,l,n,[],[],backCast,backCastAsym,stage,composite,isJoint,isInference,gScale,univariate);
ll = -ll;
Ht = zeros(k,k,T);
for t=1:T
    h = sqrt(H(t,:));
...
% TODO : Better gradient function % Gradient shit possibly not working
% anymore after my modifications
% if stage==2
%     % 1. dcc_likelihood
%     count = k*(k-1)/2 + m + l + n;
