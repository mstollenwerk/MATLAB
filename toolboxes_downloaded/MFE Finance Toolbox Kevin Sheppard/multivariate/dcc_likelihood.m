function [ll,lls,Rt,fcst_Rt] = dcc_likelihood(parameters,data,dataAsym,m,l,n,R,N,backCast,backCastAsym,stage,composite,isJoint,isInference,gScale,univariate)
...
fcst_Qt = intercept;
for i = 1:m
    fcst_Qt = fcst_Qt + a(i)*stdData(:,:,T+1-i);
end
for i = 1:l
    fcst_Qt = fcst_Qt + g(i)*stdDataAsym(:,:,T+1-i);
end
for i = 1:n
    fcst_Qt = fcst_Qt + b(i)*Qt(:,:,T+1-i);
end
q = sqrt(diag(fcst_Qt));
fcst_Rt = fcst_Qt./ (q*q');