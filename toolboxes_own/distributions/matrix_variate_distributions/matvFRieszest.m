function [ eparam, tstats, logL, optimoutput ] = matvFRieszest( data_mat, x0, varargin )
%BESSELWISHEST
%
% USAGE:
%   
...
if ~isempty(varargin) && strcmp(varargin{1},'EstimationStrategy:MethodOfMoments')
    ...
    obj_fun = @(df) matvFRieszlike( L/matvFRieszexpmat(df(1:p),df(p+1:p+p))*L', df(1:p), df(p+1:p+p), data_mat );
