function [ eparam, tstats, logL, optimoutput ] = matvtRiesz2est( data_mat, x0, varargin )
%BESSELWISHEST
%
% USAGE:
%   
...
if ~isempty(varargin) && strcmp(varargin{1},'EstimationStrategy:MethodOfMoments')
    ...
    while perm_improvement > 0
        ...
        for ii = 1:size(perm_,1)
            nLogL_permuted_assets(ii) = ...
                matvtRiesz2like( ...
                    cholU(mean(data_mat(perm_(ii,:),perm_(ii,:),:),3))/matvtRieszexpmat(eparam(1:p),eparam(p+1))*cholU(mean(data_mat(perm_(ii,:),perm_(ii,:),:),3))', ...
        ...
        obj_fun_min_ii = @(df) matvtRiesz2like( ...
            cholU(mean(data_mat(perm_(min_ii,:),perm_(min_ii,:),:),3))/matvtRieszexpmat(df(1:p),df(p+1))*cholU(mean(data_mat(perm_(min_ii,:),perm_(min_ii,:),:),3))', ...
    ...
    obj_fun = @(param) matvtRiesz2like( [], [], [], data_mat, param );

    if isempty(x0)  
        x0 = [ vech(chol(mean(data_mat,3)/2/p, 'lower')); 2*p.*ones(p,1); 2*p ];
    end

    lb = [-inf(p_,1); flipud((0:p-1)'); 2];
