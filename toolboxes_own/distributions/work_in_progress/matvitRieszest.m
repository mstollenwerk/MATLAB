...
if ~isempty(varargin) && strcmp(varargin{1},'EstimationStrategy:MethodOfMoments')
    ...
    obj_fun = @(df) matvitRieszlike( U/matviRieszexpmat(df(1:p))*U', df(1:p), df(p+1), data_mat );
    ...
    while perm_improvement > 0
        ...
        for ii = 1:size(perm_,1)
            nLogL_permuted_assets(ii) = ...
                matvitRieszlike( ...
                    cholU(mean(data_mat(perm_(ii,:),perm_(ii,:),:),3))/matviRieszexpmat(eparam(1:p))*cholU(mean(data_mat(perm_(ii,:),perm_(ii,:),:),3))', ...
        ...
        obj_fun_min_ii = @(df) matvitRieszlike( ...
            cholU(mean(data_mat(perm_(min_ii,:),perm_(min_ii,:),:),3))/matviRieszexpmat(df(1:p))*cholU(mean(data_mat(perm_(min_ii,:),perm_(min_ii,:),:),3))', ...
