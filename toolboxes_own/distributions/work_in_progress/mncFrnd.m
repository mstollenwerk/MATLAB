    function [ mncF_rnd ] = mncFrnd( df_1, df_2, Sigma_, Omega_)
...
% 21.07.2018
% 23.09.2019: fixed line 97 from if all(Omega_(:,:,ii)==0) to if all(all(Omega_(:,:,ii)==0))
...
for ii=1:N
    if all(all(Omega_(:,:,ii)==0))
        Lambda_ = zeros(p,df_1(ii));
    else
        Lambda_ = [sqrtm(Omega_(:,:,ii)), zeros(p,df_1(ii)-p)];
    end
    ...
    mncF_ = (df_2(ii) - p - 1)/df_1(ii) * sqrtm_Sigma*sqrtm_V*invT*sqrtm_V*sqrtm_Sigma;
    if max(max((abs((mncF_ - mncF_')./mncF_)))) < 1e-08
        mncF_ = (mncF_ + mncF_')/2;
