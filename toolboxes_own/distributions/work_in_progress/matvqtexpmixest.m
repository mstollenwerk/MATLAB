function [eparam, tstats, logL, optimoutput] = matvqtexpmixest(X, x0, varargin)
...
%% Optimization
if isempty(x0)
    x0 = [vech(chol(mean(X,3)/2/p,'lower'));2*p;5];
end
% Optimoptions-------------------------------------------------------------
...
[eparam,~,exitflag,optimoutput,grad,hessian] = ...
    fmincon( ...
        @(all_param) matvqtGammamixlike([],[],[],[],X,[all_param;1]), ...
        x0,[],[],[],[],lb,[],[],options ...
    );
warning('on')
...
[ nLogL, logLcontr, eparam ] = matvqtGammamixlike([],[],[],[],X,[eparam;1]);
logL.logL = -nLogL;
