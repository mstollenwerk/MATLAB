function [eparam, tstats, logL, optimoutput] = matvgbeta2est_Psi0(X, x0, varargin)
...
if isempty(x0)
    ...
    x0 = [vech(chol(x0_Omega,'lower'));x0_a;x0_b];
end
% Optimoptions-------------------------------------------------------------
...
[eparam,~,exitflag,optimoutput,grad,hessian] = ...
    fmincon( ...
        @(all_param) matvgbeta2like([],[],[],[],X,[all_param(1:p_); zeros(p_,1); all_param(p_+1:end)]), ...
        x0,[],[],[],[],lb,[],[],options ...
    );
warning('on')
...
[ nLogL, logLcontr, eparam ] = matvgbeta2like([],[],[],[],X,[eparam(1:p_); zeros(p_,1); eparam(p_+1:end)]);
logL.logL = -nLogL;
