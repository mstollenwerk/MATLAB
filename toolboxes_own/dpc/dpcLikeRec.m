function [ nLogL, nLogLcontr, S, L, d ] = dpcLikeRec( params, data, p, q )
...
paramLoad = params(1:n*(n+1)/2+2*n);
paramComp = params(n*(n+1)/2+2*n+1:end);
Sc = ivech(params(1:n*(n+1)/2),'lower');
Sc = Sc*Sc';
[Lc,dc]=dpceig(Sc);
%% Loadings Recursion
Q = rbekkRec(paramLoad, dataCell);
