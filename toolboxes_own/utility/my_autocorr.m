function varargout = my_autocorr(y,numLags,numMA,numSTD)
% !!!!!CHANGE PLOT PARAMETERS IN LINE 201 MANUALLY!!!!!
...
%  Plot the sample ACF:
   ...
   lineHandles = plot(lags,acf,'Marker', '.', 'MarkerSize', 15);%,'Color','k''k','Marker','.','LineStyle','none','MarkerSize',.1/25,'Color',[.8 .8 .8]);%,'MarkerColor',[.7 .7 .7]
...
%  null hypothesis is assumed to hold. 
   ...
   plot([numMA+0.5 numMA+0.5; numLags numLags],[bounds([1 1]) bounds([2 2])],'--k');
%    plot([0 numLags],[0 0],'-k');
   hold('off')
   a = axis;
   axis([a(1:3) 1]);
   set(gca,'XLim',[1 numLags], 'YLim', [-0.1000 0.1000])
