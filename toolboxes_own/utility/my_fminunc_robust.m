function [X,optimoutput] = my_fminunc_robust(fun,X,subgroup_idx,sensitivity_,varargin)
...
while improvement_ > sensitivity_
    ...
    [X_new,optimoutput{ii,1}] = my_fminunc(fun,X,varargin{:});
    ...
    for jj = 1:numel(indices)
        ...
        [X_new(indices{jj}),optimoutput{ii,jj+1}] = ...
                my_fminunc(fun_jj,X_new(indices{jj}),varargin{:});
