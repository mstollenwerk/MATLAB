function [eparam, tstats, logL, fit, fcst, optimoutput] = ...
	gas_scalar_har_estim( X, dist, x0, varargin )
%
%
% Michael Stollenwerk
...
%% Optimization
obj_fun = @(param) gas_scalar_har_likeRec(param, X, dist);
% x0-----------------------------------------------------------------------
if isempty(x0)
    x0 = [vechchol(meanX.*.4)' .025 .3 .2 .1];
    if strcmp( dist, 'Wish' )
		x0 = [ x0, 2*k ]';
...
elseif strcmp( dist, 'iRiesz' )
    lb = [-inf(k_+4,1)', 2:k+1]';  
elseif strcmp( dist, 'tRiesz' )
    lb = [-inf(k_+4,1)', 0:k-1, 2]'; 
elseif strcmp( dist, 'itRiesz' )
    lb = [-inf(k_+4,1)', 0:k-1, 2]';
elseif strcmp( dist, 'FRiesz' )
    lb = [-inf(k_+4,1)', (0:k-1), (k-(1:k)+2)]';    
end
A = [ zeros(1,k_) 0 1 1 1 zeros(1, length(x0)-k_-4) ];   % Stationarity
b = 1;                                                   % Stationarity
% Optimization-------------------------------------------------------------
disp('-------------------------------------------------------------------')
disp('-------------------------------------------------------------------')
disp(strcat('Starting gas scalar har ',dist,' estimation.'))
disp('-------------------------------------------------------------------')
disp('-------------------------------------------------------------------')
[eparam,optimoutput] = ...
	my_fmincon(...
    ...
    while perm_improvement > 0
        ...
        for ii = 1:size(perm_,1)
            nLogL_permuted_assets(ii) = ...
                gas_scalar_har_likeRec(eparam, X(perm_(ii,:),perm_(ii,:),:), dist);
        end
        ...
        if perm_improvement > 0
            perm_improvement
            obj_fun = @(param) gas_scalar_har_likeRec(param, X(perm_(min_ii,:),perm_(min_ii,:),:), dist);
            [eparam,optimoutput] = ...
                my_fmincon(...
                    obj_fun, ...
                    eparam, ...
...
disp('-------------------------------------------------------------------')
disp('-------------------------------------------------------------------')
disp(strcat('Finished gas scalar har ',dist,' estimation.'))
disp('-------------------------------------------------------------------')
disp('-------------------------------------------------------------------')
% Tstats-------------------------------------------------------------------
