...
k_ = k*(k+1)/2;
%% Optimization
obj_fun = @(param, perm_) obj_fun_wrapper(param, perm_, X, p, q, dist, scalingtype);
% x0-----------------------------------------------------------------------
...
logL_detR_part = -(k+1)/2*sum(logdet3d(X));
logL = struct(...
    'logL', -nLogL,...
...
function [ nLogL, logLcontr, SigmaE, ScaledScore, param_out, fitplot ] = obj_fun_wrapper(param, perm_, X, p, q, dist, scalingtype) 
