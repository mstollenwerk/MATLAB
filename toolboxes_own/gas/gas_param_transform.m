function [param_out] = gas_param_transform(dist,param,p)
...
param_out.Sig.score1 = param(p_+1:p_+p);
param_out.Sig.score2 = param(p_+p+1:p_+p+p);
param_out.Sig.garchD = param(p_+p+p+1:p_+p+p+p);
param_out.Sig.garchW = param(p_+p+p+p+1:p_+p+p+p+p);
param_out.Sig.garchM = param(p_+p+p+p+p+1:p_+p+p+p+p+p);
theta = param(p_+p+p+p+p+p+1:end);
