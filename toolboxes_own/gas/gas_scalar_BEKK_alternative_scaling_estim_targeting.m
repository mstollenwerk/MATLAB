function [eparam, tstats, logL, fit, fcst, optimoutput] = ...
	gas_scalar_BEKK_alternative_scaling_estim_targeting( X, p, q, dist, x0, varargin )
%
%
% Michael Stollenwerk
...
elseif strcmp( dist, 'iWish' )
    lb = [-inf(2*p+q,1); k+1];
elseif strcmp( dist, 'F' )
    lb = [-inf(2*p+q,1); k+1; k-1];
elseif strcmp( dist, 'tWish' )
    lb = [-inf(2*p+q,1); k-1; 2];
elseif strcmp( dist, 'itWish' )
    lb = [-inf(2*p+q,1); k+1; 0];    
elseif strcmp( dist, 'Riesz' )
    lb = [-inf(2*p+q,1)', 0:k-1]';  
elseif strcmp( dist, 'iRiesz' )
    lb = [-inf(2*p+q,1)', 2:k+1]';  
elseif strcmp( dist, 'tRiesz' )
    lb = [-inf(2*p+q,1)', 0:k-1, 2]'; 
elseif strcmp( dist, 'itRiesz' )
    lb = [-inf(2*p+q,1)', 0:k-1, 2]';
elseif strcmp( dist, 'FRiesz' )
    lb = [-inf(2*p+q,1)', (0:k-1), (k-(1:k)+2)]';    
end
A = [ zeros(1,2*p) ones(1,q) zeros(1, length(x0)-2*p-q) ];   % Stationarity
b = 1;                                                   % Stationarity
% Optimization-------------------------------------------------------------
disp('-------------------------------------------------------------------')
disp('-------------------------------------------------------------------')
