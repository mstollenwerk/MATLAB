function [eparam, tstats, logL, fit, fcst, optimoutput] = ...
	caicov_scalar_BEKK_estim( X, p, q, dist, x0, varargin )
%
%
% Michael Stollenwerk
...
%% Optimization
obj_fun = @(param) caicov_scalar_BEKK_likeRec(param, p, q, X, dist);
% x0-----------------------------------------------------------------------
if isempty(x0)
    x0 = [vechchol(meanX)' ones(1,p)*.01/p ones(1,q)*.75/q];
    if strcmp( dist, 'Wish' )
		x0 = [ x0, 2*k ]';
...
A = [ zeros(1,k_) ones(1,p+q) zeros(1, length(x0)-k_-p-q) ];   % Stationarity
b = 1;                                                      % Stationarity
% Optimization-------------------------------------------------------------
disp('-------------------------------------------------------------------')
disp('-------------------------------------------------------------------')
disp(strcat('Starting caicov scalar BEKK ',num2str(p),' ',num2str(q),' ',dist,' estimation.'))
disp('-------------------------------------------------------------------')
disp('-------------------------------------------------------------------')
[eparam,optimoutput] = ...
	my_fmincon(...
    ...
    while perm_improvement > 0
        ...
        for ii = 1:size(perm_,1)
            nLogL_permuted_assets(ii) = ...
                caicov_scalar_BEKK_likeRec(eparam, p, q, X(perm_(ii,:),perm_(ii,:),:), dist);
        end
        ...
        if perm_improvement > 0
            perm_improvement        
            obj_fun = @(param) caicov_scalar_BEKK_likeRec(param, p, q, X(perm_(min_ii,:),perm_(min_ii,:),:), dist);
            [eparam,optimoutput] = ...
                my_fmincon(...
                    obj_fun, ...
                    eparam, ...
...
disp('-------------------------------------------------------------------')
disp('-------------------------------------------------------------------')
disp(strcat('Finished caicov scalar BEKK ',num2str(p),' ',num2str(q),' ',dist,' estimation.'))
disp('-------------------------------------------------------------------')
disp('-------------------------------------------------------------------')
% Tstats-------------------------------------------------------------------
